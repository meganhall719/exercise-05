...
height_sd <-sd(z$height)
weight_sd <-sd(z$weight)
age_sd<-sd(z$age)
zombies_killed_sd <-sd(z$zombies_killed)
education_sd <- sd(z$years_of_education)
results <- data.frame(
...
print(names(sampling_dist_summary))
...
height_boot <- boot(data = sample_survivors$height, statistic = stat, R = n_boot)
weight_boot <- boot(data = sample_survivors$weight, statistic = stat, R = n_boot)
age_boot <- boot(data = sample_survivors$age, statistic = stat, R = n_boot)
zombies_boot <- boot(data = sample_survivors$zombies_killed, statistic = stat, R = n_boot)
education_boot <- boot(data = sample_survivors$years_of_education, statistic = stat, R = n_boot)


height_ci <- c(quantile(height_boot$t, 0.025), quantile(height_boot$t, 0.975))
weight_ci <- c(quantile(weight_boot$t, 0.025), quantile(weight_boot$t, 0.975))
age_ci <- c(quantile(age_boot$t, 0.025), quantile(age_boot$t, 0.975))
zombies_ci <- c(quantile(zombies_boot$t, 0.025), quantile(zombies_boot$t, 0.975))
education_ci <- c(quantile(education_boot$t, 0.025), quantile(education_boot$t, 0.975))


comparison_table <- data.frame(
  Variable = c("Height", "Weight", "Age", "Zombies Killed", "Years of Education"),
  Mean = c(
    mean(sample_survivors$height),
    mean(sample_survivors$weight),
    mean(sample_survivors$age),
    mean(sample_survivors$zombies_killed),
    mean(sample_survivors$years_of_education)
  ),
